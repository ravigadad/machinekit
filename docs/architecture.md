...
- **Capability modules** (`lib/modules/capabilities/`) are abstract slots. Each normally declares a default satisfier (platform-aware where needed) and delegates `::requires` to it; their `::install` is a no-op — the satisfier does the real work. `secrets_manager` is the exception: no universal default manager exists, so its `::requires` hard-fails with a clear recipe when required and no satisfier is listed.

**Capabilities** are the four implemented abstract slots: `tool_version_manager` (satisfied by `mise` by default), `container_manager` (satisfied by `orbstack` on macOS, `docker_ce` on Linux), `secrets_manager` (no default; satisfied by `infisical` when listed), and `postgres` (satisfied by `postgres_brew` by default — install and run PostgreSQL via Homebrew; `postgres_app` on macOS when listed — check and provision into an operator-run Postgres.app). Users list a capability in `modules = [...]` and the resolver substitutes the default satisfier, where one exists. Listing a concrete satisfier explicitly overrides the default — that's how someone says "use Docker, not OrbStack." If two satisfiers for the same capability end up in the active set, the resolver hard-fails unless `allow_multiple_satisfiers = true` is set.

**Soft (optional) dependencies.** `::requires` is a *hard* dependency — it pulls the capability into the active set and constrains install order. A module can also *opt into* a capability only when it happens to be present, without requiring it, by asking `modules::capability_active <capability>` at run time (it scans the active set for a `::provides` match). Example: the `postgres_brew` satisfier opens itself to containers in `post_apply` if a `container_manager` is active, but a plain database host that lists no container runtime stays localhost-only — it never drags a runtime in. Use a hard `::requires` when the module can't function without the capability; use the soft check when the integration is a bonus. `modules::capability_satisfier <capability>` is the same lookup but returns the satisfying module's *name* rather than true/false — needed when a capability's satisfiers have genuinely divergent implementations to dispatch to (unlike `container_manager`'s orbstack/docker_ce, which share one `docker` CLI and so never needed this). `secrets_manager` and `postgres` are its callers today: `secrets_manager`'s `::fetch`/`::ensure_ready`/`::has` each resolve the active satisfier by name, and the `postgres` capability's SQL API resolves its satisfier to dispatch the three seams that genuinely differ — `bin_dir`, `instance`, and `ensure_extension_available` (`postgres_brew` installs and runs postgres; `postgres_app` checks an operator-run Postgres.app).

**Soft ordering (`::after`).** A separate, ordering-only relationship: a module returns module names from `<name>::after` to be placed *after* each of them **when they are independently active**, without activating them. `::requires` activates-and-orders; `::after` only orders, and only among modules already in the active set (an `::after` to an inactive module is silently ignored). The resolver applies these in a second pass over the active set computed from `::requires`. Example: `agents_config_harnesses::after` returns `agents_config_setup` so the shared dir is seeded before it's projected — yet harnesses still runs standalone when the dir arrives by other means (e.g. synced in), since the edge never drags `agents_config_setup` in.

> Status: implemented. `::install`, `::preflight`, `::requires`, `::after`, and `::provides` conventions are live, plus `modules::capability_active`/`modules::capability_satisfier` for soft dependencies. `lib/modules/capabilities/` holds `tool_version_manager`, `container_manager`, `secrets_manager`, and `postgres`. `resolver.sh` handles topological sort (hard `::requires` plus soft `::after` ordering edges), capability → default-satisfier substitution (where one exists), and post-resolution conflict detection.
...
4. Interactive prompt — only if TTY and not `--non-interactive`
5. Hard error listing what's missing

One resolver handles all three execution modes without branching logic; a new bootstrap-input tier (like the per-user defaults file) adds one case and nothing else changes. A secrets manager is a related but distinct mechanism, not a tier in this cascade: it resolves a *named secret's value* (the age key, a pool secret), not a bootstrap input, and dispatches through a capability/satisfier rather than a `context::get` case — see [Secrets and key management](#secrets-and-key-management).
...
4. Otherwise, if `key_source_type = "secrets_manager"` is set under `[module.age]`, fetch the key from the active manager — its `age_key` secret, addressed by an explicit `[secrets.manager_refs]` reference or the convention name — and install it the same way. This is an explicit directive, not "a manager happens to be active": the age key is just another named secret (`age_key`), resolved through the same reference/convention machinery as everything else, so preflight fails here when the manager doesn't actually hold it rather than deferring the miss to install. If a runtime `--existing-age-key-file`/`--generate-age-key` is also passed it wins, with a warning that the configured manager source is being overridden.
5. Otherwise, hard-fail with a clear error explaining the options.

Steps 3 and 5 exist because a new key cannot decrypt files encrypted by a previous one. If a user has an existing key but forgot to copy it, silently generating a new one would lock them out of their own encrypted blueprints. Generation always requires explicit consent — and takes precedence over a manager fetch, so an explicit `--generate-age-key` mints a fresh key rather than pulling the fleet's from the manager. `key_source_type = "secrets_manager"` (step 4) is what lets a fresh machine bootstrap the shared key from the manager instead of a hand-copied file — so the only thing you seed a new box with is its manager credential; separately, a blueprint with no home-layer `.age` files and every secret manager-backed needs no age module in `modules = [...]` at all.

---

## Secrets and key management

There is one root secret: the age private key, unless a blueprint sources every secret from a manager, in which case age needn't be active at all. SSH keys are managed by machinekit's SSH module during bootstrap — installed from an existing file (`--existing-ssh-key-file`) or generated fresh (`--generate-ssh-key`) — and registered with whichever git host the user uses (via that host's normal SSH-key-add flow, which machinekit walks through interactively after generation). HTTPS credentials for the blueprints clone remain outside machinekit's scope; the user provides them via standard git mechanisms (`.netrc`, credential helper) before running.

There are two distinct channels for blueprint secrets, and the distinction matters:

- **Encrypted dotfiles** live under a `home/` layer and decrypt **1:1 to a final path in `$HOME`**. They are decrypted **during home sync** by the content-transform pipeline — not by a separate post-sync pass. The age module claims the `.age` extension as a decode-tier transform, so `secret.age` in a home layer is decrypted to its final content at `~/secret` as the file is applied. Decryption happening inside the pipeline (rather than after a verbatim copy) keeps reconcile and `--dry-run` operating on plaintext and never leaves ciphertext on disk. With the age module inactive the marker isn't registered and `secret.age` copies over as-is — unreadable but not an error. This channel is always an age-encrypted file; it has no manager-backed counterpart.

- **Ingredient secrets** are named secrets (`<service>/<name>`, e.g. `tailscale/default`) resolved via `secrets::resolve` — **backend-agnostic**: an age-encrypted pool file at `secrets/<service>/<tenancy>.age` (a sibling of `common/` and `machine_types/`, deliberately **outside the machine-type cascade**, since a secret read directly by a module participates in no layering), or a secrets-manager reference, whichever backend actually holds it. `<service>` is machinekit's vocabulary (the module that owns the secret, e.g. `tailscale`); `<tenancy>`/name suffix is the user's own label (`work`, `personal`, `default`), which natively supports multiple accounts per service per blueprint. These are **not** on the home pipeline; the consuming module calls `secrets::resolve` and decides what to do with the plaintext — keep it in memory for a one-shot use (e.g. a one-time tailnet join key) or assemble it into a mode-600 file it owns (e.g. a service's env file). What the module does with the plaintext is its choice, driven by whether the secret is needed once or on an ongoing basis. **No consuming module ever knows or branches on which backend actually resolved a secret** — that's `secrets::resolve`'s entire job.
...
**Backend resolution — `secrets::resolve` / `secrets::present` / `secrets::backend_for`.** Given a bare logical name, most-specific override wins: (1) an explicit reference configured for that name (`[secrets.manager_refs] "NAME" = "<manager-native-ref>"`), when the active secrets-manager satisfier actually holds it, is fetched from there — lets a secret be overridden per-name even when a pool file also exists, e.g. to swap an auth token while testing; an explicit reference the manager does *not* hold reports missing rather than silently falling back to the pool file it was meant to override; (2) an age-encrypted pool file at that name's path, decrypted — the default when no override says otherwise; (3) with no override and no pool file, an active secrets-manager satisfier is *asked whether it actually holds* the convention-named secret (bare name, satisfier-interpreted) and, if so, it's fetched from there; (4) otherwise missing. Presence is **truthful, never optimistic**: a merely-active manager is not enough, and neither is a merely-configured reference — every manager verdict is confirmed against the satisfier, so a name the pool lacks and the manager does not actually hold (whether by convention or by an explicit reference) reports `none`. That is what lets provide-or-generate secrets still generate when absent (rather than trying to fetch a secret that isn't there) and keeps `secrets list` from painting an absent secret as resolved. `secrets::backend_for` is this same resolution without fetching the value — reporting `pool`, `manager`, or `none` — confirming every manager verdict against the session readied in preflight (`secrets_manager::has` for a convention name, `secrets_manager::has_reference` for an explicit reference); it drives `secrets::present`, `machinekit secrets list`'s state column, and the generate-vs-fetch decision. A module's `::requires` hook does **not** use it — the manager isn't readied yet during resolution — but instead uses `secrets::backend_requirements`, a local-only classification of a module's declared secrets: `age` for any with a pool file, `secrets_manager` for any with an explicit reference. A convention-backed name needs no dependency edge (its satisfier is already listed, and readiness authenticates it before any module installs).

**The `secrets_manager` capability.** Unlike `container_manager`/`tool_version_manager`, there is no default satisfier — no universal default manager exists — so requiring it with none active (an unlisted concrete satisfier) hard-fails with a clear recipe. The stable functions every caller uses are `secrets_manager::fetch REFERENCE` (resolve a value), `secrets_manager::has NAME` / `secrets_manager::has_reference REFERENCE` (does the manager actually hold the convention-named / explicitly-referenced secret — the truthful existence checks behind `backend_for`), and `secrets_manager::ensure_ready` (install and authenticate the satisfier); each looks up the active satisfier and dispatches to its contract function, so no caller ever names a concrete manager. `ensure_ready` runs once during preflight — and for `machinekit secrets list` and an interactive `machinekit secrets put` — *before* any module resolves a secret, so presence is knowable while validating; the resulting token / keyring session then carries through the run. Because readiness authenticates ahead of every `run_installs`, a manager-backed consumer can never install before an unauthenticated manager, whatever order the resolver picks. First satisfier: **`infisical`** (Infisical Cloud only — self-hosting is a separate, undone topic). Two auth methods (`module.infisical.auth_method`, typically machine-type layered): `universal` — a machine identity (`client_id` + `client_secret`) exchanges for a short-lived access token via `infisical login`, the only path that works non-interactively; `user` — browser/SSO login, interactive-only (federated SSO has no password to hand over programmatically, and a prior session's OS-keyring entry isn't portable to a new machine), with **readiness** failing immediately if run on a non-interactive machine rather than deep inside an impossible login. Credentials never reach the process table: the client id/secret and access token are passed to the CLI through environment variables, never argv. Readiness — including the login, and for `user` auth possibly a browser prompt — runs even under `--dry-run`, since truthful presence needs it; it logs that authenticating is the one outward action a dry-run takes.

**Knowing what's needed — `machinekit secrets`.** A module that draws on named secrets declares them with a `<name>::declared_secrets` hook: resolved against current config, it emits one `name · required · can_be_generated` row (bare logical name, e.g. `tailscale/default` — no `.age` suffix, no pool prefix) per secret it will actually use on this machine (so a *tagged* tailscale device lists its join key while an untagged one lists nothing). `machinekit secrets list` aggregates those declarations across the active modules and joins each against `secrets::backend_for` — reporting each secret as required-or-not, generated-if-missing-or-not, and resolved via `pool` / `manager` / `missing`, with any pool file no active module claims flagged separately as a likely typo or stale entry. It **applies nothing to the machine**, but when a secrets manager is active it readies it — a network loop, and for `user` auth possibly a browser login — so a manager-backed secret shows its *true* source rather than a guess; it answers "what does this blueprint need, and what's still missing" before a run. The required/can-be-generated split is two independent booleans by design — a secret can be required but un-generatable (you must provide the LLM key), required and generatable (machinekit mints the tenant key if absent), and the model leaves room for a future merely-optional secret.

**Putting a secret into the pool — `machinekit secrets put [<name>]`.** The write-side complement, deliberately **pool/age-only** — a manager-backed secret is populated in the manager directly, not through here. It age-encrypts a value and places it as an `.age` file, so propagating a fleet secret isn't a bespoke `age -r … -o …` pipe each time. The recipient defaults to the public key derived from the installed identity (override with `--recipient`); the value comes from piped stdin, `--from-file`, or an interactive hidden prompt — **never an argument**, to keep plaintext out of shell history. When `--from-file` points at a file that is *already* age-encrypted, machinekit files it as-is instead of re-encrypting it (which would double-encrypt): it verifies the installed key can decrypt it — age files don't reveal their recipients, so a trial decryption is the only check, and it proves *this* machine can read the file (in the default single-identity fleet, that the fleet can) — then copies the ciphertext verbatim. A `--recipient` override can't apply to a copy, so it is rejected rather than silently ignored; failing to decrypt means only that the local key isn't a recipient, not that the file is invalid. The target is a bare logical name — resolved to `secrets/<service>/<name>.age` (exactly as `list` prints it) against a local blueprint working tree (`--blueprints-dir`, defaulting to a local `blueprints.source`) — or an absolute path, written as-is (an escape hatch). With no target and an interactive tty, it presents the inventory as a picker. Replacing an existing secret requires an `--overwrite` confirmation (flag / env / prompt). Per the [explicit-consent rule](#the-explicit-consent-rule) and machinekit's "doesn't touch git history" posture, it **only writes to a working tree** — it never commits, pushes, or writes to the throwaway apply-time fetch cache; you commit and push to propagate. For a remote (git-URL) `blueprints.source` there is no local tree machinekit knows, so `--blueprints-dir` (your clone) is required.

> Status: implemented. The age module manages the key (including sourcing it from a secrets manager, above) and supplies the `.age` decode handler (the home-pipeline channel). `secrets::resolve`/`present`/`backend_for` (`lib/machinekit/secrets.sh`) are the backend-agnostic channel; every module that declares secrets (via a `::declared_secrets` hook) goes through them rather than calling `age::decrypt` directly, and their `::requires` hooks share `secrets::backend_requirements`. The `secrets_manager` satisfier is readied (`secrets_manager::ensure_ready`) in preflight, for `secrets list`, and for an interactive `secrets put`, and existence is checked against it (`secrets_manager::has`), so presence is truthful. `machinekit secrets list` reports the inventory with each secret's resolved backend, and `machinekit secrets put` encrypts-and-places a secret — or verifies-and-copies an already-encrypted file — into a working tree (over the `age::recipient` / `age::encrypt` / `age::can_decrypt` primitives), never touching git, and never handling manager-backed secrets (out of its scope by design).
...
A module that spans several files keeps the extra ones in a same-named subdirectory and sources them itself (e.g. `postgres_brew.sh` sources `postgres_brew/introspect.sh`). `modules::source_all` auto-loads only top-level `lib/modules/*.sh` files plus capability satisfiers in `capabilities/`; subdirectory files are a module's private implementation, reached through its own `source` lines, not discovered as modules.
