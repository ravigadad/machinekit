...
> Status: implemented. `::install`, `::preflight`, `::requires`, `::after`, and `::provides` conventions are live, plus `modules::capability_active` for soft dependencies. `lib/modules/capabilities/` holds `tool_version_manager` and `container_manager`. `resolver.sh` handles topological sort (hard `::requires` plus soft `::after` ordering edges), capability → default-satisfier substitution, and post-resolution conflict detection.
