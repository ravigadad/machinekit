...
- Zsh hook pattern — the `zsh` module's env.zsh ends with a glob-source loop over the `env.zsh.d/*.zsh` dir in machinekit's config dir (`${XDG_CONFIG_HOME:-$HOME/.config}/machinekit`). Modules that need zsh setup ship a fragment in their `templates/xdg_config/machinekit/env.zsh.d/`. mise uses this today; future modules plug in the same way.
...
Shipped: `machinekit secrets list` reports which secrets the active modules need — each as required-or-not, generated-if-missing-or-not, and resolved via `pool`, `manager`, or `missing` — plus any stray pool files no active module claims. Modules declare their needs through a `<name>::declared_secrets` hook, using bare logical names (e.g. `tailscale/default`) rather than literal pool paths. It applies nothing to the machine, but when a secrets manager is active it authenticates to it (a network loop, possibly a browser login) so a manager-backed secret's source is reported truthfully, not guessed.

Also shipped, the write side: `machinekit secrets put [<name>]` age-encrypts a value (stdin / `--from-file` / hidden prompt — never an argument) and places it as a pool secret at that name's `.age` path, with an interactive picker over the inventory when no name is given. It writes only to a local blueprint working tree — never git, never the apply-time fetch cache — so you commit and push to propagate. `put` is deliberately pool/age-only: a manager-backed secret is populated in the manager directly, not through here.

### Backend-agnostic secret resolution

Shipped: `secrets::resolve` / `secrets::present` / `secrets::backend_for` (`lib/machinekit/secrets.sh`) resolve any named secret — an age-encrypted pool file, or (given an active `secrets_manager` satisfier) a secrets-manager reference — without the consuming module ever knowing which backend actually holds it. `[secrets.manager_refs]` maps a name to an explicit manager-native reference; absent that, an active manager is queried by convention name and used only if it actually holds the secret (presence is truthful, so an absent secret reads as missing rather than falsely manager-backed). The satisfier is readied — installed and authenticated — during preflight, for `secrets list`, and for an interactive `secrets put`, before any secret is resolved. The age key itself gained a parallel third sourcing path alongside `--existing-age-key-file` and interactive generation, so a fully manager-sourced blueprint needs no age key at all. First satisfier: `infisical` (Infisical Cloud only; self-hosting is a separate, undone topic), supporting both machine-identity (`universal`) and browser/SSO (`user`) auth — the latter interactive-only, since federated SSO has no password to hand over non-interactively.
