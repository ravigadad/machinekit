...
- Additional modules as needs arise. Shipped since the module system landed: `tailscale` (mesh networking, with headless tagged-device support), a `postgres` host-provisioning module, `claude_code` (installs the Claude Code CLI), and the Hindsight memory stack — `hindsight_server` (self-hosted memory API as a container); more as needed (prompt themes, password-manager CLIs as a secrets-fetch layer, and so on). The current set lives in [`lib/modules/`](../lib/modules/), with operator setup for the ones that need it in [docs/modules.md](./modules.md).
