...
- **Stub vs. let-it-run (London-school; a hard boundary):** stub when the collaborator owns the outcome or has external/expensive side effects (network, real git, another module). Let it run *only* for a side effect you dictate — e.g. a `mkdir`/`cp` to a path the test controls — and assert the result directly. A value the collaborator *computes* (a mapping, a format, a derivation) is always the stub case, even when it is pure and side-effect-free; a parent test whose expected value is only right because the real helper ran is the integrated anti-pattern. See the global testing-principles `[stub_what_the_collaborator_owns]`.
