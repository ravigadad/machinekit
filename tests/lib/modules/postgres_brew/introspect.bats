#!/usr/bin/env bats
# Tests for lib/modules/postgres_brew/introspect.sh

load "${BATS_TEST_DIRNAME}/../../../test_helper"

setup() {
  # shellcheck source=../../../../lib/modules/postgres_brew/introspect.sh
  source "$MACHINEKIT_DIR/lib/modules/postgres_brew/introspect.sh"
}

# --- postgres_brew::introspect::formula ---

@test "formula builds the versioned formula name" {
  run postgres_brew::introspect::formula 14
...
@test "formula fails when called without a version" {
  ...
  run postgres_brew::introspect::formula
  [ "$status" -ne 0 ]
  [ -z "$output" ]
...
@test "installed_versions lists installed postgres majors, stripping the formula prefix" {
  ...
  run postgres_brew::introspect::installed_versions
  [ "$output" = $'14\n17' ]
}
...
@test "is_installed delegates to brew with the versioned formula name" {
  STUB_OUTPUT="fake-formula" mktest::stub_function postgres_brew::introspect::formula "14"
  mktest::stub_function brew::_is_installed "formula" "fake-formula"
  postgres_brew::introspect::is_installed 14
...
@test "is_installed is false when brew reports the formula absent" {
  ...
  run ! postgres_brew::introspect::is_installed 14
}
...
@test "latest_available_version resolves the postgresql alias to its major via brew metadata" {
  ...
  run postgres_brew::introspect::latest_available_version
  [ "$output" = "18" ]
}
...
@test "data_dir locates the cluster under the brew prefix var directory" {
  ...
  STUB_OUTPUT="fake-formula" mktest::stub_function postgres_brew::introspect::formula "14"
  run postgres_brew::introspect::data_dir 14
...
@test "bin_dir resolves the keg-only bin directory for the version" {
  ...
  run postgres_brew::introspect::bin_dir 14
  [ "$output" = "/fake/keg/bin" ]
}
...
@test "pg_ctl_path points at pg_ctl inside the version's bin directory" {
  STUB_OUTPUT="/fake/bin" mktest::stub_function postgres_brew::introspect::bin_dir "14"
  run postgres_brew::introspect::pg_ctl_path 14
...
@test "is_initialized is true when the cluster has a PG_VERSION stamp" {
  ...
  postgres_brew::introspect::is_initialized 14
}
...
@test "is_initialized is false when the data directory lacks PG_VERSION" {
  ...
  STUB_OUTPUT="$datadir" mktest::stub_function postgres_brew::introspect::data_dir "14"
  run ! postgres_brew::introspect::is_initialized 14
}
...
@test "configured_port reads an explicit uncommented port from postgresql.conf" {
  ...
  STUB_OUTPUT="$datadir" mktest::stub_function postgres_brew::introspect::data_dir "14"
  run postgres_brew::introspect::configured_port 14
  [ "$output" = "5433" ]
}
...
@test "is_running is true when pg_ctl reports the cluster up" {
  STUB_OUTPUT="fake_pg_ctl" mktest::stub_function postgres_brew::introspect::pg_ctl_path "14"
  STUB_OUTPUT="/fake/datadir" mktest::stub_function postgres_brew::introspect::data_dir "14"
  mktest::stub_function fake_pg_ctl "status" "-D" "/fake/datadir"
  postgres_brew::introspect::is_running 14
...
@test "is_running is false when pg_ctl reports the cluster down" {
  ...
  run ! postgres_brew::introspect::is_running 14
  mktest::assert_stub_called fake_pg_ctl "status" "-D" "/fake/datadir"
}
...
@test "instance_version picks the initialized major configured on port 5432" {
  STUB_OUTPUT=$'14\n17' mktest::stub_function postgres_brew::introspect::installed_versions
  ...
  run postgres_brew::introspect::instance_version
  [ "$status" -eq 0 ]
  [ "$output" = "17" ]
}
...
@test "instance_version skips an installed-but-uninitialized major despite the 5432 port default" {
  STUB_OUTPUT="18" mktest::stub_function postgres_brew::introspect::installed_versions
  ...
  [ "$status" -ne 0 ]
  [ -z "$output" ]
  mktest::assert_stub_not_called postgres_brew::introspect::configured_port
}

@test "instance_version fails when no installed major is on 5432" {
  STUB_OUTPUT="14" mktest::stub_function postgres_brew::introspect::installed_versions
  mktest::stub_function postgres_brew::introspect::is_initialized "14"
  STUB_OUTPUT="5433" mktest::stub_function postgres_brew::introspect::configured_port "14"
  run postgres_brew::introspect::instance_version
  [ "$status" -ne 0 ]
...
@test "instance_version fails when no postgres is installed" {
  STUB_OUTPUT="" mktest::stub_function postgres_brew::introspect::installed_versions
  run postgres_brew::introspect::instance_version
  [ "$status" -ne 0 ]
  [ -z "$output" ]
}
