#!/usr/bin/env bats
# Tests for lib/modules/postgres_brew/access.sh

load "${BATS_TEST_DIRNAME}/../../../test_helper"

setup() {
  # shellcheck source=../../../../lib/modules/postgres_brew/access.sh
  source "$MACHINEKIT_DIR/lib/modules/postgres_brew/access.sh"
...
@test "configure opens access on Linux" {
  ...
  mktest::stub_function postgres_brew::access::_open_for_containers
  postgres_brew::access::configure
  mktest::assert_stub_called postgres_brew::access::_open_for_containers
}
...
@test "_open_for_containers opens listen, authorizes the subnet, then restarts" {
  ...
  mktest::stub_function postgres_brew::access::_open_listen "17"
  mktest::stub_function postgres_brew::access::_authorize_subnet "17" "172.30.0.0/16"
  mktest::stub_function postgres_brew::_restart "17"
  postgres_brew::access::_open_for_containers
  mktest::assert_stub_called_in_order postgres_brew::access::_open_listen "17"
  mktest::assert_stub_called_in_order postgres_brew::access::_authorize_subnet "17" "172.30.0.0/16"
  mktest::assert_stub_called_in_order postgres_brew::_restart "17"
}
...
@test "_open_for_containers fails loudly when no instance is resolved" {
  ...
  run postgres_brew::access::_open_for_containers
  [ "$status" -ne 0 ]
...
@test "_open_for_containers in dry-run reports without touching docker or the config" {
  ...
  mktest::stub_function container_manager::container_subnet
  mktest::stub_function postgres_brew::access::_open_listen
  mktest::stub_function postgres_brew::_restart
  postgres_brew::access::_open_for_containers
  mktest::assert_stub_called logging::dry_run
  mktest::assert_stub_not_called container_manager::container_subnet
  mktest::assert_stub_not_called postgres_brew::access::_open_listen
  mktest::assert_stub_not_called postgres_brew::_restart
}

# --- postgres_brew::access::_open_listen ---

@test "_open_listen writes the listen_addresses block to postgresql.conf" {
  STUB_OUTPUT="/fake/d" mktest::stub_function postgres_brew::introspect::data_dir "17"
  mktest::stub_function postgres_brew::access::_write_block \
    "/fake/d/postgresql.conf" "listen_addresses = '*'"
  postgres_brew::access::_open_listen 17
  mktest::assert_stub_called postgres_brew::access::_write_block \
    "/fake/d/postgresql.conf" "listen_addresses = '*'"
}
...
@test "_authorize_subnet writes a scram pg_hba line scoped to the subnet" {
  ...
  postgres_brew::access::_authorize_subnet 17 "172.30.0.0/16"
  mktest::assert_stub_called postgres_brew::access::_write_block \
    "/fake/d/pg_hba.conf" "host all all 172.30.0.0/16 scram-sha-256"
}
...
@test "_write_block writes a marked block idempotently, preserving other lines" {
  ...
  printf 'existing = yes\n' > "$f"
  postgres_brew::access::_write_block "$f" "listen_addresses = '*'"
  postgres_brew::access::_write_block "$f" "listen_addresses = '*'"
...
@test "_write_block replaces a stale block rather than appending a second one" {
  local f="$BATS_TEST_TMPDIR/conf"
  : > "$f"
  postgres_brew::access::_write_block "$f" "old = 1"
  postgres_brew::access::_write_block "$f" "new = 2"
  run grep -c '^old = 1$' "$f"
  [ "$output" = "0" ]
  run grep -c '^new = 2$' "$f"
  [ "$output" = "1" ]
